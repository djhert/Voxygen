...
public class RoadCube : Shape {
	public RoadCube() : base() {
	}
	...
	public override Vector3 [] MakeShape( float x, float y, float z, WorldTypes.Direction direction, float size = 1.0f, int placement=1, int rotation=0  ) {
		return CreateRoadCube(x,y,z,direction);
	}
	
	public Vector3[] CreateRoadCube (float x, float y, float z, WorldTypes.Direction direction)
	{
		Vector3 [] verts = new Vector3[4];
		switch(direction) {
		case WorldTypes.Direction.up:
			verts [0] = addtoCube(x,y,z,Shape.getVert(0,0.1f));
			verts [1] = addtoCube(x,y,z,Shape.getVert(4,0.1f));
			verts [2] = addtoCube(x,y,z,Shape.getVert(24,0.1f)); 
			verts [3] = addtoCube(x,y,z,Shape.getVert(20,0.1f)); 
			break;
